	...
	class OpenFile
	{
		...
		static int Main( string[] args )
		{
			...
			using ( OpenFileDialog dialog = new OpenFileDialog( ) )
			{
				...
				{
					bool solved = false;
					// Check if there are too many arguments because of trailing backslash followed by doublequote in folder
					string pattern = @"^\s*(\""?)([^\""]+)\1\s+(\""?)([^\""]+)\3\s+(\""?)([^\""]+)\5\s+(\""?)([^\""]+)\7\s*$";
					Regex regex = new Regex( pattern );
					if ( regex.IsMatch( Environment.CommandLine ) )
					{
						MatchCollection matches = regex.Matches( Environment.CommandLine );
						if ( matches.Count == 1 )
						{
							Match match = matches[0];
							if ( match.Groups.Count == 9 )
							{
								filter = match.Groups[4].Value;
								folder = match.Groups[6].Value;
