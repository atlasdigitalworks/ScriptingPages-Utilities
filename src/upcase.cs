...
namespace RobvanderWoude
{
	class UpCase
	{
		...
		{
			bool verbose = false;
			char[] locaseletters = "abcdefghijklmnopqrstuvwxyz".ToCharArray( );
			...
			else
			{
				...
				foreach ( string arg in args )
				{
					...
					else
					{
						...
						{
							if ( Path.GetFileName( file ).IndexOfAny( locaseletters ) > -1 )
							{
			...
			if ( Console.IsInputRedirected )
			{
				#region Convert redirected input to upper case
				...
				#endregion Convert redirected input to upper case
			}
			else
			{
				#region Rename files to upper case
				...
				foreach ( string file in files )
				{
					if ( File.Exists( file ) )
					{
						...
						{
							string newfilename = Path.Combine( parentfolder, filename.ToUpperInvariant( ) );
							try
							{
								if ( verbose )
								{
				...
				#endregion Rename files to upper case
			}
		}
