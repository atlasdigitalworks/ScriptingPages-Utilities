...
namespace RobvanderWoude
{
	class DropDownBox
	{
		public static string progver = "1.17";
		...
		static int Main( string[] args )
		{
				...
				else
				{
					switch ( arg.Substring( 0, 3 ).ToUpper( ) )
					{
						...
						case "/D:":
							...
							{
								return ShowHelp( "Invalid delimiter specified \"{0}\"", arg );
							}
							break;
						case "/F:":
							if ( listset )
							...
							else
							{
								try
								{
									...
									list = Regex.Replace( list, pattern, delim );
									pattern = "(^" + delim + "|" + delim + "$)";
									list = Regex.Replace( list, pattern, string.Empty );
								}
								catch ( Exception e )
								{
									return ShowHelp( e.Message );
						...
						case "/H:":
							...
							try
							{
								...
								{
									return ShowHelp( "Height {0} outside allowed range of {1}..{2}", arg.Substring( 3 ), defaultheight.ToString( ), Screen.PrimaryScreen.Bounds.Height.ToString( ) );
								}
							}
							catch ( Exception e )
							{
								return ShowHelp( "Invalid height \"{0}\": {1}", arg.Substring( 3 ), e.Message );
							}
							heightset = true;
							break;
						case "/I:":
							...
							{
								return ShowHelp( "Invalid index value \"{0}\": {1}", arg, e.Message );
							}
							break;
						case "/L:":
							if ( localizedcaptionset )
						...
						case "/T:":
							...
							try
							{
								...
								{
									return ShowHelp( "Tab length {0} outside allowed range of 4..16", arg.Substring( 3 ) );
								}
							}
							catch ( Exception e )
							{
								return ShowHelp( "Invalid tab length \"{0}\": {1}", arg.Substring( 3 ), e.Message );
							}
							tablengthset = true;
							break;
						case "/W:":
							...
							try
							{
								...
								{
									return ShowHelp( "Width {0} outside allowed range of {1}..{2}", arg.Substring( 3 ), defaultwidth.ToString( ), Screen.PrimaryScreen.Bounds.Width.ToString( ) );
								}
							}
							catch ( Exception e )
							{
								return ShowHelp( "Invalid width \"{0}\": {1}", arg.Substring( 3 ), e.Message );
							}
							widthset = true;
							break;
						default:
			...
			{
				return ShowHelp( "Selected index ({0}) outside list range (0..{0})", selectedindex.ToString( ), listrange.ToString( ) );
			}
