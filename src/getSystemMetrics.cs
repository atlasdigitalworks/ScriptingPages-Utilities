...
namespace RobvanderWoude
{
	class GetSystemMetricsWrapper
	{
		...
		static int Main( string[] args )
		{
			...
			bool listall = true;
			bool openurl = false;
			bool specificmetrics = false;
			List<string> metricargs = new List<string>( );
			...
			foreach ( string arg in args )
			{
				switch ( arg.ToUpper( ) )
				{
					...
					default:
						...
						metricargs.Add( arg );
						specificmetrics = true;
						listall = false;
						break;
				}

				if ( !listall && !specificmetrics )
				{
					return ShowHelp( "Specify at least one metric, or /L to list all" );
				}

				if ( listall && specificmetrics )
				{
					return ShowHelp( "Either specify one or more metrics, or /L to list all, but not both" );
				}
			}
			...
			int rc = -1;
			string url = "https://msdn.microsoft.com/library/windows/desktop/ms724385.aspx";
			...
			{
				foreach ( string metricarg in metricargs )
				{
					// Get a single system metric value
					SystemMetric systemmetric;
					if ( Enum.TryParse<SystemMetric>( metricarg, true, out systemmetric ) )
					{
						rc = GetSystemMetrics( systemmetric );
						Console.WriteLine( "{0}={1}", Enum.GetName( typeof( SystemMetric ), systemmetric ), rc );
					}
					else
					{
						if ( metricargs.Count > 1 )
						{
							return ShowHelp( "Invalid metrics" );
						}
						else
						{
							return ShowHelp( "Invalid metric" );
						}
					}
				}
				if ( metricargs.Count > 0 )
				{
					rc = 0;
				}
			}

			// Open the URL with the list of possible metric values
			if ( openurl )
			{
		...
		static void ListAllMetrics( )
		{
			SystemMetric systemmetric;
			// List all metric names and sort the list by name
			List<string> metricnames = new List<string>( Enum.GetNames( typeof( SystemMetric ) ) );
			metricnames.Sort( );
			// Determine longest metric name, to make all metric names fit in output table
			int maxstringlength = 0;
			foreach ( string metricname in metricnames )
			{
				if ( metricname.Length > maxstringlength )
				{
					maxstringlength = metricname.Length;
				}
			}
			// Show table head
			Console.WriteLine( "{0,-" + maxstringlength + "}    Index:              Value:", "Metric Name:" );
			Console.WriteLine( "{0,-" + maxstringlength + "}    ======              ======", "============" );
			// Show each metric name, index and value
			foreach ( string metricname in metricnames )
			{
				string metricvalue = new String( ' ', 20 );
				if ( Enum.TryParse<SystemMetric>( metricname, true, out systemmetric ) )
				{
					metricvalue = String.Format( "{0,20}", GetSystemMetrics( systemmetric ) );
				}
				if ( Enum.TryParse<SystemMetric>( metricname, true, out systemmetric ) )
				{
					Console.WriteLine( "{0,-" + maxstringlength + "}    {1,6}{2}", metricname, (int) systemmetric, metricvalue );
				}
				else
				{
					Console.WriteLine( "{0,-" + maxstringlength + "}         ?{1}", metricname, metricvalue );
				}
			}
		}
		...
		static int ShowHelp( params string[] errmsg )
		{
			...
			/*
			GetSystemMetrics.exe,  Version 1.03
			Wrapper for the WINAPI (user32.dll) GetSystemMetrics function
			
			Usage:    GetSystemMetrics.exe  [ metric [ metric [..] ] | /L ]  [ /U ]
			
			Where:    metric     is the name or index of a SystemMetric to be queried
			                     (multiple metrics allowed)
			          /L         List all metrics names, indexes and values, sorted by name
			...
			Notes:    If a single metric is specified, the program's return code will
			          ...
			          https://msdn.microsoft.com/library/windows/desktop/ms724385.aspx
			
			Credits:  SystemMetric enumeration published by Gabriel T. Sharp on PInvoke.net
			...
			*/
			...
			Console.Error.WriteLine( "GetSystemMetrics.exe,  Version {0}", progver );

			Console.Error.WriteLine( "Wrapper for the WINAPI (user32.dll) GetSystemMetrics function" );

			Console.Error.WriteLine( );

			Console.Error.Write( "Usage:    " );
			Console.ForegroundColor = ConsoleColor.White;
			Console.Error.WriteLine( "GetSystemMetrics.exe  [ metric [ metric [..] ] | /L ]  [ /U ]" );
			Console.ResetColor( );
			...
			Console.ForegroundColor = ConsoleColor.White;
			Console.Error.Write( "metric" );
			Console.ResetColor( );
			Console.Error.WriteLine( "     is the name or number of a SystemMetric to be queried" );

			Console.Error.WriteLine( "                     (multiple metrics allowed)" );

			Console.ForegroundColor = ConsoleColor.White;
			Console.Error.Write( "          /L         L" );
			Console.ResetColor( );
			Console.Error.WriteLine( "ist all metrics names, numbers and values, sorted by name" );

			Console.Error.WriteLine( "                     (default, implemented for backwards compatibility only)" );
			...
			Console.Error.Write( "U" );
			Console.ResetColor( );
			Console.Error.WriteLine( "RL with list of metric values and their meanings" );

			Console.Error.WriteLine( );

			Console.Error.WriteLine( "Notes:    If a single metric is specified, the program's return code will" );

			Console.Error.WriteLine( "          equal the value returned by the WINAPI GetSystemMetrics function," );

			Console.Error.WriteLine( "          for multiple metrics the return code will be 0, in case of" );

			Console.Error.WriteLine( "          (command line) errors the return code will be -1." );

			Console.Error.WriteLine( "          The meaning of the returned metric values can be found online at" );

			Console.ForegroundColor = ConsoleColor.DarkGray;
			Console.Error.WriteLine( "          https://msdn.microsoft.com/library/windows/desktop/ms724385.aspx" );
			Console.ResetColor( );

			Console.Error.WriteLine( );

			Console.Error.WriteLine( "Credits:  SystemMetric enumeration published by Gabriel T. Sharp on PInvoke.net" );

			Console.ForegroundColor = ConsoleColor.DarkGray;
			Console.Error.WriteLine( "          http://pinvoke.net/default.aspx/Enums/SystemMetric.html" );
			Console.ResetColor( );
